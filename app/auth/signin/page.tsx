...
export default function SignInPage() {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = error.code === "auth/invalid-credential" ? "Invalid email or password" : error.message || "Sign in failed";
      toast.error(message);
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Ensure user document exists (in case they signed in before signing up)
      ...
      <div className="w-full max-w-[380px] sm:max-w-[400px]">
        ...
        <div className="card-premium p-5 sm:p-7 bg-surface-1 border border-border shadow-xl">
          ...
          <div className="text-center mb-5 sm:mb-6">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-text-1 mb-1">Welcome back</h1>
            <p className="text-xs sm:text-sm text-text-3">Sign in to your account</p>
          </div>
          ...
          <form onSubmit={handleEmailSignIn} className="space-y-3 sm:space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="text-xs font-semibold text-text-2">Password</label>
                <Link href="/auth/reset-password" className="text-xs text-gold hover:text-gold/80 transition-colors">
                  Forgot?
                </Link>
              </div>
              <div className="relative">
                <input
                  ...
                  disabled={loading}
                  minLength={6}
                  className="w-full px-3 py-2 sm:py-2.5 pr-10 rounded-lg border border-border bg-surface-2 text-text-1 text-sm placeholder:text-text-3 focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold transition-all disabled:opacity-50"
                />
                <button
                  type="button"
